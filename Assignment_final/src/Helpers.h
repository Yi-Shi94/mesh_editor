...
public:
    ...
    int selected_face = -1;
    vector<int> selected_vertex_index;
    
    Vector3d barycenter;
    Vector3d color;
    vector<Vector3d> normals_phong;
    vector<vector<int>> vert_neigbors;
    vector<vector<int>> face_neigbors;
    ...
    MatrixXf populate_vertex();
    MatrixXf populate_normal_vertex();
    void set_color(Vector3d color);
    void set_render_type(int r_type);
    
    vector<Vector3d> load_vertices(int i);
    vector<Vector3d> load_tris(int i);
    vector<Vector3d> load_faces(int i);
    
    vector<Vector3d> get_vertices();
    vector<Vector3d> get_tris();
    vector<Vector3d> get_faces();
    
    void update_tris();
    void select_vertex_2(float x,float y,MatrixXf inv_trans);
    void select_vertex(Vector3d ray_origin,Vector3d ray_direction,double t);
    bool delete_vertex();
    bool translate_vertex(int axis,double amount);
    bool merge_vertex();
    bool check();
    int save_off();
    
    vector<int> is_edge(int vert_ind_a, int vert_ind_b);
    int get_render_type();
